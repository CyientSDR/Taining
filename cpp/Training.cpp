...
PREPARE_LOGGING(Training_i)

Training_i::Training_i(const char *uuid, const char *label) :
    Training_base(uuid, label)
{
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void Training_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &Training_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &Training_i::<callback method>)
        in the constructor.
        ...
        Training_i::Training_i(const char *uuid, const char *label) :
            Training_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &Training_i::scaleChanged);
            addPropertyListener(status, this, &Training_i::statusChanged);
        }

        void Training_i::scaleChanged(float oldValue, float newValue)
        {
            RH_DEBUG(this->_baseLog, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void Training_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
...
************************************************************************************************/
int Training_i::serviceFunction()
{
